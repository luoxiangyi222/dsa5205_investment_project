...
def train(stock_name, term_length, folder_name, data_description):
    ...
    if data_description == 'rfe_pca':
        ...
        df = pd.read_csv(f'../trading_strategy_data/{folder_name}/{stock_name}_pca_data.csv')
        ...
        history = lstm.fit(train_X_arr, train_Y_arr, batch_size=batch_size, epochs=epochs)
        ...
        # prepare plot data
        train_set = data.iloc[:-validation_length].copy()
        train_set['pred'] = train_pred_Y
        valid_set = data.iloc[-validation_length:].copy()
        ...
        if data_description == 'all':
            df = pd.read_csv(f'../trading_strategy_data/{folder_name}/{stock_name}_combined_data.csv')
        elif data_description == 'rfe':
            df = pd.read_csv(f'../trading_strategy_data/{folder_name}/{stock_name}_rfe_data.csv')
...
if __name__ == "__main__":
    ...
    for stock in crawler.MOMENTUM_PORTFOLIO:
        train(stock, 2, folder_name='portfolio_data/momentum', data_description='rfe_pca')
