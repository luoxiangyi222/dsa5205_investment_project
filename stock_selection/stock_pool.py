...
# counting.to_csv('count.csv', index=True)
...
combined = pd.read_csv('/Users/huaerli/Desktop/2021 AYS1/DSA5205/group-project-local/code/data/stock_pool_init.csv', index_col= 0)
print(combined)
...
for c in intraday.columns:
    ...
    ind[c] = temp
print(ind)
tickers = [t for t in tickers if ind[t] <= 5]
print(len(tickers))
# data = data[tickers]
...
for c in drawdown.columns:
    ...
    temp = len(drawdown[(drawdown[c] > 0.3) | (drawdown[c] < -0.3)])
    ind[c] = temp
print(ind)
tickers = [t for t in tickers if ind[t] <= 20]
print(len(tickers))
data = data[tickers]
